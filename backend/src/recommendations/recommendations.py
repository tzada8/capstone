...
class Recommendation:
    ...
    def _scrape_bestbuy() -> Dict:
        ...
        if len(trending.get("results")) == 0 or len(most_popular.get("results")) == 0:
            ...
            for t in trending.get("results"):
                master_list[t.get("sku")] = t.get("rank") + MAX_BESTBUY_RANK
            for p in most_popular.get("results"):
                if p.get("sku") in master_list.keys():
                    master_list[p.get("sku")] = master_list.get(p.get("sku")) - MAX_BESTBUY_RANK + p.get("rank")
                else:
                    master_list[p.get("sku")] = p.get("rank") + MAX_BESTBUY_RANK
            # Change SKU to UPC.
            skus = ""
            ...
            if "error" in results or results.get("total") == 0:
                ...
                return upc_master_list

    @staticmethod
    def _scrape_walmart() -> Dict:
        # TODO: Add something if anything can be found
        pass

    @staticmethod
    def _model(preferences: Dict, importance: Dict, selected_products: List, master_list = Dict) -> List:
        max_rank = len(selected_products) + 1
        max_master_list_rank = len(master_list) + 1
        recommendations = []
        ...
        for p in selected_products:
            ...
            actual_brand = "" if len(brand) == 0 else brand[0].get("value")

            megapixels = list(filter(lambda spec: spec["name"] == pixels_key, p.get("specifications")))
            actual_megapixels = 0 if len(megapixels) == 0 else int(re.findall(r'\d+', megapixels[0].get("value"))[0])

            lens_type = list(filter(lambda spec: spec["name"] == lens_type_key, p.get("specifications")))
            actual_lens_type = "" if len(lens_type) == 0 else lens_type[0].get("value")

            camera_type = list(filter(lambda spec: spec["name"] == "Digital Camera Type", p.get("specifications")))
            actual_camera_type = "" if len(camera_type) == 0 else camera_type[0].get("value")

            actual_price = float(p.get("basic_info").get("price").get("amount"))
            ...
            if preferences.get("brand") == "Canon" or preferences.get("brand") == "Nikon":
                brand_diff = 0 if actual_brand == preferences.get("brand") else 1
            elif preferences.get("brand") == "Other":
                brand_diff = 0 if actual_brand != "Canon" and actual_brand != "Nikon" else 1
            ...
            if preferences.get("lens_type") == "Fixed" or preferences.get("lens_type") == "Standard": 
                lens_type_diff = 0 if preferences.get("lens_type") in actual_lens_type else 1
            elif preferences.get("lens_type") == "Other":
                lens_type_diff = 0 if actual_lens_type != "Fixed" and actual_lens_type != "Standard" else 1
            else: # If no option is selected.
                lens_type_diff = 0

            # Camera Type: [Point and Shoot, DSLR, Mirrorless, Other]
            # TODO: Check this more with potential types from Best Buy and Walmart.
            if preferences.get("camera_type") == "Point and Shoot" or preferences.get("camera_type") == "DSLR" or preferences.get("camera_type") == "Mirrorless": 
            ...
            rank_weight = 0.25
            pref_weight = 0.75
            worst_score = pref_weight * sum(range(1, 6)) + rank_weight * ((MAX_BESTBUY_RANK + max_rank) * 2)

            # Prepare all products for table.
            recommendations.append({
                "product_id": p.get("basic_info").get("product_id") if p.get("basic_info").get("source") == "Best Buy" else p.get("basic_info").get("us_item_id"),
    ...
    def aggregate_data(preferences: Dict, importance: Dict, selected_products: List) -> List:
        ...
        list = Recommendation._model(preferences, importance, selected_products, master_list)
        return list
